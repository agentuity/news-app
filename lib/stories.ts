...
export async function getPublishedStories(): Promise<Story[]> {
	...
	return stories.sort((a, b) => {
		const dateA = new Date(a.date_published ?? a.date_added).getTime();
		const dateB = new Date(b.date_published ?? b.date_added).getTime();
		return dateB - dateA;
	});
}
...
export async function getStoriesByDateRange(
	...
	for (const date of dates) {
		pipeline.smembers(getDateKey(date));
	}
	const responses = await pipeline.exec();
	...
	return stories.sort((a, b) => {
		const dateA = options.publishedOnly
			? new Date(a.date_published ?? a.date_added)
			: new Date(a.date_added);
		const dateB = options.publishedOnly
			? new Date(b.date_published ?? b.date_added)
